...
for k = 1:length(h) %Resolve as equações para cada passo no vetor h
    ...
    for i = 1:N % Loop para executar as iterações do método de Runge Kutta de 2° ordem
        k1 = f(y(i,:)); % K1 = f(Yi)
        k2 = f(y(i,:)+h(k)*k1); % K2 = f(Yi + h(k)*k1)
        p(i,:) = k1; %passa para o vetor de plotagem
        y(i+1,:) = y(i,:) + (h(k)/2)*(k1+k2); % Y(i+1) = Y(i) + h(k)/2*(k1+k2)
    ...
    Plot('RK2', k, p, t, precision, h); %chama a função Plot
end